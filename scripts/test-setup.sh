...
log_test "setup-deploy with Azure VM + Attached Storage"
cleanup
make setup-deploy \
    ...
    AZURE_PERMANENT_RG=keystone-permanent-rg \
    ...
    KEYCLOAK_ADMIN=admin \
    KEYCLOAK_ADMIN_PASSWORD=admin-secret \
    CADDY_BUCKET_NAME=not-used-for-azure \
    > /dev/null 2>&1
check_config_value .deploy-config "ENVIRONMENT" "prod"
check_config_value .deploy-config "CLOUD_PROVIDER" "azure"
check_config_value .deploy-config "CLOUD_REGION" "francecentral"
check_config_value .deploy-config "INFRA_TYPE" "vm-attached-storage"
check_config_value .deploy-config "DOMAIN_NAME" "azure.example.com"
check_config_value .deploy-config "AI_PROVIDER" "openai"
check_config_value .deploy-config "AZURE_SUBSCRIPTION_ID" "00000000-0000-0000-0000-000000000000"
check_config_value .deploy-config "AZURE_PERMANENT_RG" "keystone-permanent-rg"
